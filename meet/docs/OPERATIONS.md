...
- **My availability** — Your free times on the calendar grid. Use **Days shown** (3 / 5 / 7; 7 only when weekends are included) to fit the screen. Mark day headings to **Copy days** / **Paste** / **Invert days**. Copy keeps a full day sequence (including blank days) as shown left-to-right; Paste fills from the first marked destination onward. On touch, swipe the grid to move dates.
- **Confirm meeting choices** — Group calendar overlap view and organiser scheduling. Click a start to save it immediately (click again to clear). Confirm location(s). Empty hours and days can be hidden. Same **Days shown** toggle as My availability. Organiser status required to edit.
